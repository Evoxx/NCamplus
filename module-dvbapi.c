			...
			uint32_t filter_number, uint8_t *data, struct s_client *client, ECM_REQUEST *er, uint16_t client_proto_version)
{
	...
	switch(request)
	{
		case DVBAPI_SERVER_INFO:
		{
			...
			*info_len = snprintf((char *) &packet[size], sizeof(packet) - size, "NCam v%s, build r%s (%s); %s",
...
static void dvbapi_parse_pmt_descriptors(int32_t demux_id, uint8_t *buffer, uint16_t length, uint8_t *type)
{
	...
	for(i = 0; i + 1 < length; i += 2 + descriptor_length)
	{
		...
		switch(descriptor_tag)
		{
			...
			{
				uint8_t extension_descriptor_tag = buffer[1 + 2];
			...
			case 0x81:
			{
				...
				{
					demux[demux_id].ens = b2i(4, buffer + 1 + 2);
					demux[demux_id].tsid = b2i(2, buffer + 1 + 6);
					demux[demux_id].onid = b2i(2, buffer + 1 + 8);
	...
	*pmtpid = 0x00;
	...
	while(pos + 1 < 5 + program_info_length)
	{
		...
		switch(descriptor_tag)
		{
			...
			case 0x85: // service type mask (not used by NCam)
				break;
				...
				if(g == demux[i].ECMpidcount)   // all usable pids are tried, lets start over again!
				{
					if(demux[i].decodingtries == -1) // first redecoding attempt?
					{
						...
						for(g = 0; g < demux[i].ECMpidcount; g++)
						{
							...
							// remove this pid from channelcache since we had no founds on any ecmpid!
...
uint32_t dvbapi_ca_set_pid(int32_t demux_id, int32_t pid, int32_t stream_id, bool use_des, uint32_t msgid)
{
	...
	if(demux[demux_id].ECMpids[pid].useMultipleIndices)
	{
		...
		{
			dvbapi_set_pid(demux_id, n, idx, true, use_des, msgid); // enable streampid
		}
		else
		{
			dvbapi_set_pid(demux_id, n, idx, false, false, msgid); // disable streampid
		}
	}
	else
	{
		...
		for(n = 0; n < demux[demux_id].STREAMpidcount; n++)
		{
			if(!demux[demux_id].ECMpids[pid].streams || ((demux[demux_id].ECMpids[pid].streams & (1 << n)) == (uint) (1 << n)))
			{
				dvbapi_set_pid(demux_id, n, idx, true, use_des, 0); // enable streampid
			}
			else
			{
				dvbapi_set_pid(demux_id, n, idx, false, false, 0); // disable streampid
			}
		}
	}
